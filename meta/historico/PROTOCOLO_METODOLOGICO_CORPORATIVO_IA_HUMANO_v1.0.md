...
## 4. PROCESSO DE CRIAÇÃO DE PROTOCOLOS
...
## 6. PIPELINE METODOLÓGICO PADRÃO
```mermaid
flowchart LR
A[Ideação] --> B[Modelagem de Contexto]
B --> C[Geração de Derivados]
C --> D[Validação IA Auditor]
D --> E[Junta Multidisciplinar]
E --> F[Publicação e Versão]
F --> G[Feedback e Aprendizado Iterativo]
```
...
validation_team:
  ...
  auditor: "IA_Auditor_v2"
```

### 8.4 Processo de Validação
```mermaid
flowchart LR
IADeveloper --> Junta --> Orquestrador --> IAArchitect --> IAAuditor --> Release
```
...
- Nenhuma entrega sem dupla validação.  
...
```mermaid
sequenceDiagram
IADeveloper->>Junta: Envia entrega para validação
Junta->>Orquestrador: Reporta inconsistências
Orquestrador->>IAArchitect: Define ajustes
IAArchitect->>IADeveloper: Repassa correções
IAAuditor->>Junta: Valida nova versão
```
...
- Auditorias automáticas e humanas executadas por ciclo.  
- Qualquer divergência gera um *diff-report* automatizado.

---

## 11. INFRAESTRUTURA DE EXECUÇÃO (IA INFRA)
